...
function [means, Covariances] = meanandvar_forcat2(Training_Samples,unseen,category_order,num_classes, looseness_constraint)
...
for k = 1:length(unseen)
    ...
    for j = 1:size(new_category_order,1)
        ...
        same_idx = find(new_category_order(j,:) == attr_rank);
        
        % Find if there are any elements with the same values
        if isempty(same_idx) == 0
            % Get the elements that are the same
            same_means = 0;
            for a = 1:length(same_idx)
                same_mean = means(1,j,seen(same_idx(a)));
            end
            same_mean = same_mean/length(same_idx);
            means(1,j,class) = same_mean;
